...
class GoogleGeminiClient(LLMClient):
    ...
    async def generate(self, prompt: str, model: Optional[str] = None) -> str:
        ...
        logger.debug(f"Usando Google Gemini para gerar resposta")
        ...
        try:
            ...
            if hasattr(primeira_parte, 'finish_reason') and primeira_parte.finish_reason == types.FinishReason.SAFETY:
                ...
                raise APIError("Erro: Sua pergunta não pôde ser processada devido às diretrizes de segurança.")
            ...
            raise APIError(f"Timeout: A requisição demorou mais de {settings.api_timeout_seconds} segundos.")
            ...
            raise AgentError(f"Erro: Ocorreu uma falha inesperada.")
...
class OpenRouterClient(LLMClient):
    ...
    async def generate(self, prompt: str, model: Optional[str] = None) -> str:
        ...
        try:
            ...
            raise APIError(f"Timeout: A requisição ao OpenRouter demorou mais de {self.timeout} segundos.")
            ...
            if status_code == 429:
                ...
                logger.error(f"Erro de autenticação no OpenRouter")
                raise APIError("Erro: Chave API do OpenRouter inválida.")
            else:
                logger.error(f"Erro HTTP {status_code} do OpenRouter: {e}")
                raise APIError(f"Erro: Ocorreu um problema de comunicação com o OpenRouter (HTTP {status_code}).")
...
async def generate_with_fallback(
    ...
    try:
        ...
        for client in fallback_clients:
            if isinstance(client, OpenRouterClient) and fallback_models:
                ...
                for model in fallback_models:
                    try:
                        logger.info(f"Tentando fallback com {client.get_provider_name()} modelo '{model}'")
        ...
        raise APIError("Todos os provedores e modelos disponíveis falharam. Por favor, tente novamente mais tarde.")
