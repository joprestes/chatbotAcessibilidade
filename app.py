...
def call_adk_agent(agent_instance: Agent, full_prompt_text: str, agente_nome_log: str) -> str:  # Renomeado para full_prompt_text
    ...
    try:
        ...
        return f"Erro de atributo ao executar agente ADK '{agente_nome_log}': {e}. Verifique a API do Runner/Event do ADK. Detalhes: {e}"
...
def validador_links(texto_com_links: str) -> str:
    if texto_com_links.startswith(("Erro ao executar agente ADK", "Erro no agente ADK", "Agente ADK")):
        return "Validação de links não realizada devido a um erro na etapa anterior."
    links = re.findall(r'https?://\S+', texto_com_links)
    if not links:
        return "Nenhum link encontrado para validação."
    resultados = []
    for link in links:
        ...
        try:
            ...
            resultados.append(f"⚠️ {link_limpo} (Timeout)")
        except requests.exceptions.RequestException:
            resultados.append(f"❌ {link_limpo} (Quebrado/inacessível)")
