...
def test_detect_injection_patterns_nao_retorna_falsos_positivos(texto):
    ...
    assume(not any(pattern in texto.lower() for pattern in ['<script', 'javascript:', 'onerror=']))
    assume(not any(pattern in texto for pattern in ['SELECT', 'DROP', 'INSERT', 'UPDATE', 'DELETE']))
    assume('--' not in texto)
...
@pytest.mark.property
@given(st.text(min_size=3, max_size=100, alphabet=st.characters(whitelist_categories=('L', 'N', 'P', 'Z'))))
