...
async def test_chat_flow_deep_integration_success(client):
    ...
    with patch("chatbot_acessibilidade.pipeline.orquestrador.get_agent_response", side_effect=mock_get_agent_response) as mock_agent:
...
async def test_chat_flow_deep_integration_validator_correction(client):
    ...
    with patch("chatbot_acessibilidade.pipeline.orquestrador.get_agent_response", side_effect=mock_get_agent_response):
...
async def test_chat_flow_deep_integration_parallel_failure(client):
    ...
    with patch("chatbot_acessibilidade.pipeline.orquestrador.get_agent_response", side_effect=mock_get_agent_response):
        ...
        assert "Não foi possível gerar sugestões de testes" in data["resposta"]["🧪 **Como Testar na Prática**"]
